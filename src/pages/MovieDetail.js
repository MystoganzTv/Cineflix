  ...
  useEffect(() => {
    async function fetchMovie() {
      const response = await fetch(
        `https://api.themoviedb.org/3/movie/${params.id}?api_key=4e83d00b047da632f66dec04686c8fc4`
      );
      const json = await response.json(); // Converting the response into a json
      setMovie(json); //utilize the info with json
      console.log(json);
    }
    fetchMovie();
  }, [params.id]);
